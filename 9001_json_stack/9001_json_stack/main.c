...
StackResult stackPush(Stack* stack, char bracket, int position){
    ...
    stack->top += 1;
    stack->data = (StackElem*)realloc(stack->data, stack->capacity * sizeof(StackElem));
    if (stack->data == NULL) {
        return STACK_MEM_ERR;
    }
...
int jsonBracketCheckBasic(const char *json_str) {
    ...
    if (strlen(json_str) == 0 || json_str == NULL) {
        return 1;
    }
    ...
    for (int i = 0; i < strlen(json_str); i++) {
        if (isLeftBracket(json_str[i])) {
            ...
            StackElem elem;
            if (stackPop(&stack, &elem) != STACK_OK || !isBracketMatch(elem.bracket, json_str[i])) {
                result = 0;
            }
        }
    }
    ...
    for (int i = 0; i < len; i++) {
        if (in_string) {
            if (escape == 1) {
                ...
                escape = 1;
            }else if (json_str[i] == '"' && escape == 0) {
                in_string = 0;
            }
        }else{
            if (json_str[i] == '"') {
                in_string = 1;
            } else if (isLeftBracket(json_str[i])) {
                stackPush(&stack, json_str[i], i);
            } else if (isRightBracket(json_str[i])) {
                StackElem elem;
                if (stackPop(&stack, &elem) != STACK_OK || !isBracketMatch(elem.bracket, json_str[i])) {
...
// 步骤5：遍历结束后，检查栈是否为空且不在字符串内，两者都满足则返回1，否则返回0
    if (!stackIsEmpty(&stack) || in_string) {
        stackDestroy(&stack);
        return 0;
    }
