...
int load_records(Record rec[], int max_count)
{
    ...
    FILE* f_ptr = fopen(SCORE_FILE, "r");
    if (f_ptr == NULL) {
        return 0;
    }
    while (count < max_count && fscanf(f_ptr, "%s %d %s %s", rec[count].name, &rec[count].ms, rec[count].date, rec[count].time_str) != EOF) {
        count++;
    }
    fclose(f_ptr);
    return count;
...
void show_records(const Record rec[], int count)
{
    ...
    printf("No Name                Time (ms)    Date      Time\n");
    printf("------------------------------------------------------\n");
    for (int i = 0; i < count; i++) {
        printf("%-2d %-19s %-10d %-10s %-8s\n", i+1, rec[i].name, rec[i].ms, rec[i].date, rec[i].time_str);
    }
}
...
int add_record(Record rec[], int *count, int ms)
{
    ...
    if (pt != NULL)
    {
        sprintf(rec[*count].date, "%04d-%02d-%02d",
                pt->tm_year + 1900,
                pt->tm_mon + 1,
                pt->tm_mday);
        sprintf(rec[*count].time_str, "%02d:%02d:%02d",
                pt->tm_hour,
                pt->tm_min,
                pt->tm_sec);
    }
    rec[*count].ms = ms;
    (*count)++;
    FILE *f_ptr = fopen(SCORE_FILE, "a");
    if (f_ptr == NULL) {
        return -1;
    }
    fprintf(f_ptr, "%s %d %s %s\n", rec[*count-1].name, rec[*count-1].ms, rec[*count-1].date, rec[*count-1].time_str);
    fclose(f_ptr);
    return 1;
}
